...
\subsection{Landing Pad Table}\label{sec:landing-pad-table}
...
\subsection{Abstract State}\label{sec:abstract-state}
...
% \begin{itemize} % Freek didn't like the inlining with labels and it looks bad without them so just turning into plain list
  %     \item $\bool$ and $\natnum$ denote Booleans and natural numbers respectively;
  ...
  %     \item program termination type $\termination\coloneqq\{\bot,\good,\bad\}$.
  % \end{itemize}
$\bool$ and $\natnum$ denote Booleans and natural numbers respectively;
...
%\subsubsection{$\rmap$: the register map}
...
%\subsubsection{$\stack$: the return address stack.}
...
\subsubsection{$\emap$: the exception mapping.}
...
\subsubsection{$\terminated$: the termination state.}
...
\subsubsection{Auxiliary exception variables.}
$\absState$ also contains a count of the number of currently-uncaught exceptions ($\uncaught\colon\natnum$) and a stack of the currently-caught exception IDs ($\caught\colon[\word]$).
...
%\subsubsection{The abstract state record restated:} % Freek didn't like this so leaving out
\begin{definition}
...
\subsection{Abstract Transition Rules}\label{sec:step-function}
...
% Going back to two-column alignment for LNCS as I'll probably need the space
\begin{align*}
...
% Had to move the figure here to get it on the desired page!
\begin{figure*}
  \centering
...
% Trying to save space!
%\subsubsection{Non-Unwinding Rules}
...
\subsection{Argument for Overapproximation}
