...
% We therefore restate the definition of an \emph{$\alpha$-simulation}\cite{clarke94,loiseaux1995property}.
% This definition coincides exactly with the standard notion of a behavioural simulation\cite{milner1971algebraic}, but is formulated in terms of the concepts used in abstract interpretation.
...
% Abstract interpretation requires the definition of a Galois connection $\alphagamma$ that respectively does abstraction and concretization~\cite{cousot1977abstract,cousot1996abstract}.
% A Galois connection can be used to construct an overapproximative abstraction of a transition system, by using~$\alpha$ as a simulation-relation and $\gamma$ to map properties on abstract states back to properties over the concrete system~\cite{yavuz16}.
...
% Based on these, functions $\alpha$ and $\gamma$ are straightforward (more formal details can be found in~\cite{yavuz16}).
...
% Under that assumption, $T_\mathsf{C} \sqsubseteq T_\mathsf{A}$ coincides with a behavioral simulation sufficiently strong to preserve properties of the universal fragment of computation tree logic ($\forall$CTL)\cite{baier2008principles} pertaining to exceptional control flow.
...
%A technique addressing this challenge is \emph{predicate abstraction}~\cite{clarke94,graf97}.
...
%Yavuz et al. provide formal definitions for $\alphagamma$ for abstract interpretation based on predicate abstraction~\cite{yavuz16}.

%Correctness, then, is provided by Lemma~3 of~\cite{yavuz16}.
%Having the list of concrete state predicates~$\Pi$, let $\alphagamma$ be the Galois connection as defined in~\cite{yavuz16}.
...
\index{exception handling!structured}.
