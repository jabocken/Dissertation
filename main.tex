...
\includeonly{hg-formulation,hg-results,hg-discussion,hg-lifting}
...
%  \maketitle
  ...
  \part{\emph{Formally Verified Lifting of C-compiled \glsentryname{arch} Binaries}}\label{hg}
  \include{hg-lifting}
...
%    \printindex % Interferes with glossaries-extra index
    \printunsrtglossary[style=index] % Default glossary (this stuff normally goes in back matter); for whatever reason the default style doesn't format @index/child @entries right (extra desc, not including name)
