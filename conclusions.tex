...
Certain properties, such as memory usage and control flow-related ones, can only be proven on the assembly level.
This is due to
\begin{enumerate}
  \item memory usage requiring a concrete representation of memory and
...
\begin{enumerate}
  \item \emph{semi-automated} methodologies for memory usage and later
...
This dissertation presented two methods for proving memory usage,
...
but with a focus on memory usage specifically.
...
Such \iac{scf} is used as one of the proof ingredients for a generated \ac{fmuc}.
...
It produces \iac{hg} that contains:
\begin{enumerate}
...
As a formal property, memory usage has been proven to never miss any memory regions written to, assuming the correctness of the semantics and model it is applied to \autocite{bockenek2019preservation,verbeek2022lifting}.
...
In other words, stronger invariants mean stricter memory usage proofs.
...
\subsection{Concurrency and Interrupts and Signals}
...
Therefore, in order to increase reliability and trustworthiness of \cref{hg,eicfg}, full integration with \iac{itp} approach is desirable.
...
For the \ac{eicfg} work, this would involve providing \iac{itp} infrastructure for validating the work as a whole given the validation of individual steps.
...
This may involve the development of a formal library for semantics that is specifically intended to lift machine code or assembly to \iac{ir} designed for the various tasks presented in this dissertation.
