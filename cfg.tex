...
The main goal of the approach is to prove the property of \emph{memory usage},%
\index{memory!usage}.
...
My three main contributions to the memory usage work presented here
were:
\begin{enumerate*}
  \item the development of a tool for producing proof skeletons
  for memory usage, interfacing with the analysis tool
  ...
  for invariants that provide for function-level composition (\cref{se:cfg_composition});
  and
...
By building on the binary analysis tool \texttt{angr} \autocite{shoshitaishvili2016state},%
\index{\texttt{angr}}
...
a tweaked version of the \texttt{angr}
\lstinline|reassembly| analysis \cite{wang2017ramblr}
provided by \textcite{roessle2019}\footnote{%
  This is required to use the Isabelle parser provided by that work.%
}.%
\index{\texttt{angr}!\texttt{reassembly}}

To achieve minimal symbolic execution,
it features \emph{cutpoints}%
\index{cutpoint}
automatically selected by a Python script that relies on
...
Tail call recursion can help with that \autocite{clinger1998tail},%
...
\index{cutpoint}
also generates skeleton memory usage theories for every function analyzed.
...
\begin{figure}
  ...
  \begin{tikzpicture}[node distance=4.5cm, auto, ->, >=stealth, myblock/.style={
    ...
    \node[myblock, right of=isa] (mem) {Formal Proof of Memory Usage};
...
from the HermitCore unikernel library \autocite{lankes2016hermitcore}.%
\index{HermitCore}%
\index{unikernel}
...
A formal definition of memory usage requires a formal basis to work with,
...
\begin{definition}[Hoare triple for memory usage]\label{def:htriple}
  $\htriple{P}H{Q}$
...
\begin{proof}
  ...
  and~$l_f$, Floyd \cite{floyd1967assigning} showed that a Floyd invariant that holds
...
The formal definition of memory usage%
\index{memory!usage}
...
Memory usage formulates that any byte not within any of the regions in~$M$
...
\begin{definition}[Memory usage]\label{mem_pres_hoare}
  ...
  A piece of assembly demonstrates \emph{memory usage} if and only if,
...
Starting the current program fragment from a state that both satisfies
the specified precondition and assumes that each byte has some value
and then executing that program fragment to the specified halting condition
will result in a state that satisfies the specified postcondition
and retains the same value for that byte.
...
In order to prove memory usage, \lstinline{rbp} must be shown to be preserved.
...
For the memory usage proof of this function, we chose to associate annotations
...
provide us with our theorem of memory usage.
...
The unikernel library HermitCore \cite{lankes2016hermitcore} was chosen%
\index{HermitCore}
...
either ones included with Isabelle or ones from the \ac{afp} \cite{afp}
...
of memory usage in x86-64 binaries,
...
Requirements for memory usage invariants are provided for several examples.
...
Memory usage was formally proven for functions with loops, recursion,
