...
The \gls{memuse} analysis approach presented in this chapter
provides \pgls{floyd} methodology featuring annotations on specific instructions.
It is designed for the proving of \gls{memuse}
over individual assembly functions.
That property ensures that only documented regions of memory are read and written;%
\index{memory!region}
anything outside of those regions remains unchanged.

This \gls{floyd} verification
...
\begin{comment}
  My three main contributions to the \gls{memuse} work presented here were:
  \begin{enumerate*}
    \item the development of a tool for producing proof skeletons
    for \gls{memuse}, interfacing with the analysis tool
...
\index{\texttt{angr}}
That modified version was provided by \textcite{roessle2019verified}
for generating assembly usable with their \gls{isabelle} parser.%
\index{\texttt{angr}!\texttt{reassembly}}
By building on \texttt{angr},
the work of abstracting from binary to \gls{cfg} is handled with minimal user input.
...
\index{cutpoint}
also generates skeleton \gls{memuse} theories for every function analyzed.
The theory files can then be opened in the interactive theorem prover \gls{isabelle}
and the assembly loaded using the parser of \textcite{roessle2019verified}.
...
in order to complete the proofs of \gls{memuse}.
Defining the necessary invariants for functions with complex control flow
is generally a hard task, but targeting a property such as \gls{memuse}
...
\begin{figure}
  \centering
  \begin{tikzpicture}[node distance=4.5cm, auto, ->, myblock/.style={
    draw,
    text width=2.5cm,
    text centered,
    rounded corners,
    ...
    \node[myblock, right of=cfg] (isa) {\gls{isabelle}};
    \node[myblock, right of=isa] (mem) {Formal Proof of \gls{memuse}};
  ...
  \end{tikzpicture}
  \caption{Overview of \gls{floyd} \gls{memuse} verification}\label{fig:cfg_overview}
\end{figure}

\section{Formal Definitions}\label{se:cfg_defs}
A formal definition of \gls{memuse} requires a formal basis to work with,
...
which is extracted from the current state based on the value of the instruction pointer \rip.
...
\index{function!run}
$\gls{run}:(S\rightarrow\gls{bool})\times S\rightarrow(S\mid\err\mid\infloop)$.
...
Formally, this is achieved by a standard \gls{lfp} construction.

\subsection{Hoare Triples for Memory Usage}
...
\begin{definition}[Hoare triple for \gls{memuse}]\label{def:htriple}
  $\htriple{P}H{Q}$
...
and therefore not usable within \gls{isabelle}[/HOL].
...
\begin{definition}[Floyd invariant]\label{def:floyd_inv}
  ...
  \end{equation}
  where
  $\sigma'=\gls{run}((\lambda\sigma_r\cdot I(\loc\sigma_r)\neq\gls{bot}),\sigma)$%
  and $\loc\sigma_r$ is the current program location,
  stored in \rip\ on \gls{arch} systems.
\end{definition}
...
\begin{proof}
  ...
  which is trivial.
\end{proof}
In essence, \gls{floyd} verification
models a program as \pgls{cfg} where each edge is an implication.

\subsection{Definition of Memory Usage}\label{sse:mem_pres}
The formal definition of \gls{memuse}
...
\end{proof}
\begin{example}[Compositionality]
  ...
  As long as programs~$x$ and~$y$ do not contain \lstinline|goto|s
...
\begin{example}[Function calls]
  ...
  and the stack pointer, \rsp, will have some value $\rspo$.
  ...
  (a task performed by \inlineasm{call}):
  $\readmem{\rspo}{8}=l_\var{call}+5\land\mathrsp=\rspo$.
  Second, the postcondition must imply that after \inlineasm{ret},
  ...
  Note that \inlineasm{call} itself decrements the stack pointer by~8,
...
registers, such as \rbp\ and \inlineasm{r12-r15}.
It is generally assumed that the net effect of a function call
does not touch these registers.
Consider a situation in which \rbp\ contains an address,
which will be used as the target of a write after a function call.
In order to prove \gls{memuse}, \rbp\ must be shown to be preserved.
...
leaving those functions in the \gls{tcb}.
...
this will result in the proper \rsp\ restoration.
...
\subsection{Non-Recursive Loop Example: \texorpdfstring{\lstinline|pow2|}{\texttt{pow2}}}\label{sse:pow2_example}
This simple loop-based function, shown in \cref{lst:pow2},
raises two to the power of its argument.
The assembly code was obtained by compiling \pgls{c} program containing the function
with \gls{gcc} 7.2.0
and disassembling it using the modified \lstinline|reassembly| analysis
mentioned in \cref{se:cfg_overview}. The input is stored in \reg{edi}.
\begin{lstlisting}[
  language=C,
  gobble=2,
  float=*,
  caption=\lstinline|pow2| in \gls{c},
  label=lst:pow2
]
...
\begin{lstlisting}[
  ...
  gobble=2,
  float=*,
  caption=\lstinline|pow2| in \gls{arch} assembly,
  label=lst:pow2a,
  firstnumber=0
]
  pow2:
...
\begin{enumerate*}%[label=\alph*.)]
  \item the caller's \rbp\ at $\mathrbp=\rspo-8$ (eight bytes);
...
For the \gls{memuse} proof of this function, we chose to associate annotations
...
\Cref{fig:pow2cfg} shows the Floyd invariant in \gls{cfg} form for this function.
The invariant carries through the preservation of memory,
showing that region $\region{a}{1}$ maintains its value throughout.
The equalities over \rbp\ and \rsp\ are used by the memory region reasoner.
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Floyd invariant for \lstinline|pow2| in \glsfmtshort{cfg} form}\label{fig:pow2cfg}
\end{figure}
...
the result of a function compiled with \gls{gcc}[ 7.2.0]
...
stored in~\reg{dil} (the lowest eight bits of \reg{edi}/\rdi).
\begin{lstlisting}[
  style=x64,
  caption=\Gls{arch} assembly of factorial example,
  label=lst:factasm,
  float,
  firstnumber=0,
  gobble=2
...
though in this case that value specifically refers to the value of \rsp\ for the first/topmost call to the recursive function.
...
\begin{figure*}
  \centering
  \begin{tikzpicture}
    \graph[math nodes, grow down=4cm]{
      a/"1:\begin{array}{l}
  ...
  \end{tikzpicture}
  \caption{Floyd invariant for factorial in \glsfmtshort{cfg} form}\label{fig:factcfg}
\end{figure*}
...
pushing values onto the stack until \reg{dil} becomes~0.
...
This is done by instantiating variable~$n$ with \reg{dil}.
...
provide us with our theorem of \gls{memuse}.
...
This bypasses the need for a separate \gls{os} \autocite{madhavapeddy2014unikernels},
...
unikernels are intended for use in the same situations as traditional \glspl{vm}
...
Designed for the \gls{arch} \gls{isa}, HermitCore is mostly written in C.
...
\begin{table*}
  ...
  \begin{threeparttable}
    ...
    \begin{tabular}{lSSSSSSSS S[table-figures-integer=2]}
      \toprule
      \thead{Functions} & {\thead{Count}} & {\thead{\glsxtrshort{sloc}}} &
...
\Cref{fig:dequeue_push,fig:buddy_large_avail} show the \glspl{cfg} for two of
...
\begin{figure*}
  ...
  \end{subfigure}
  \caption{Floyd invariants for the described case study functions in \glsfmtshort{cfg} form}
\end{figure*}
...
\begin{enumerate*}
  ...
  \item finishing the proof of \gls{memuse}.
\end{enumerate*}
...
While restricting the verification effort to \gls{memuse}
...
that may affect \gls{memuse} must be kept track of as well.
...
In most cases, that proof can be handled by \gls{isabelle}[/HOL]
using standard off-the-shelf libraries,
either ones included with \gls{isabelle} or ones from the \gls{afp}
...
requiring significant \gls{itp} over word arithmetic.

\section{Summary}
This chapter covered one method for formal verification
of \gls{memuse} in \gls{arch} binaries,
...
Requirements for \gls{memuse} invariants were provided for several examples.
For recursive functions, more involved invariants are required,
along with \gls{itp} to show preservation of the stack and frame pointers.
...
The approach was applied to functions of HermitCore, a unikernel~\gls{os}.
\Gls{memuse} was formally proven for functions with loops, recursion,
\gls{c} structs and unions, and dynamic memory operations.
