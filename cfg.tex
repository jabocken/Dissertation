...
$\step:S\rightarrow(S\mid\err)$.%
\nomenclature[type]{$S$}{Type representing program state; an Isabelle record}%
...
\index{function!run}
$\run:(S\rightarrow\bool)\times S\rightarrow(S\mid\err\mid\infloop)$.
...
$I:L\rightharpoonup(S\rightarrow\bool)$.%
\nomenclature[type]{$L$}{Type of instruction addresses in a program; a 64-bit word}%TODO: maybe change this to \mathbb{P}? probably should at least be \mathbb{L}
\nomenclature[type]{$\bool$}{Type of boolean values, True and False}
...
\begin{definition}[Floyd invariant]\label{def:floyd_inv}
  ...
  \begin{equation}
    ...
    \sigma'\neq\err\wedge(\sigma'=\infloop\vee I(\loc\sigma')(\sigma')),
  \end{equation}
  where
...
\begin{figure}
  ...
  \begin{tikzpicture}[>=stealth]
    \graph[math nodes, grow down=2.2cm]{
      "1:\begin{array}{l}
        \boxed{\readmem{a}{1}=v_0}\wedge\mathrbp=\rbpo\wedge\mathrsp=\rspo\wedge{} \\
      ...
      \end{array}" ->[
        ...
        \boxed{\readmem{a}{1}=v_0}\wedge\mathrbp=\mathrsp=\rspo-8\wedge{} \\
      ...
      ] "\retaddr:\boxed{\readmem{a}{1}=v_0}\wedge\mathrbp=\rbpo\wedge\mathrsp=\rspo+8";
