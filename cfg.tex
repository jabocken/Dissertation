...
to prove the previously-mentioned property of memory preservation%
...
The methodology can be applied to functions with loops and subcalls,
...
the methodology documents the assumptions required to prevent it.
For efficiency, it structures the \emph{Floyd invariant} for a function
such that every path through the program is symbolically executed%
\index{symbolic execution}
only once.

A further overview of control-flow-driven verification
can be seen in \cref{se:cfg_overview},
...
is disassembly of an x86-64 binary containing it
using a modified version of the \lstinline|reassembly| analysis
...
the verification methodology features \emph{cutpoints}%
\index{cutpoint}
automatically selected by a Python package that relies on
...
This significantly complicates the necessary invariants
...
optimization, implemented in most major C and C++ compilers, can help with that
by converting recursive implementations into loop-based ones that only use
one stack frame for the recursion.
Unfortunately, that only applies when the functions under consideration use
the appropriate formulations proper tail calls require \autocite{probst2001proper}.
Neither of the recursive functions examined in this chapter met those requirements.
...
would require some additional assumptions in the analysis.
