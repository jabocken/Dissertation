...
To read a region of memory in the state, we use the notation
$\readmemS{\sigma}{a}{s}$.
If it is clear from context which state is meant, that state will be omitted.
This function reads the list of~$s$ bytes starting from the given address
and converts it to a word.
One notation for writing to memory that is used in this dissertation is
$\sigma\llparenthesis a \writeM v\rrparenthesis$,
which denotes writing a word~$v$ to address~$a$ in state~$\sigma$.
This function decomposes the given word into bytes and then writes them into memory
in the appropriate order.
As size information is encoded in the type of~$v$, an explicit size is not necessary.




% TODO: some should go in CFG section as the Hoare triple formulation is specifically for that
\section{Memory Preservation}\label{se:memory_preservation}
\todo{move to CFG section}
