...
\begin{example}\label{ex:simple}
  ...
  \end{equation*}
  where $\takebits{h,l}w$ indicates taking the bits in word~$w$ from bit number~$l$
  to bit number~$h$ using 0-indexing
  and~$\concat$ indicates bitstring concatenation.%
  \nomenclature{$\takebits{h,l}w$}{Indicates taking the bits in word~$w$ from bit 	number~$l$ to bit number~$h$ using 0-indexing}%
...
\begin{definition}
  ...
  notation $r_0\separate r_1$, if and only if
  \begin{equation*}
