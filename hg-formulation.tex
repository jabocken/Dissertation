...
This \lcnamecref{ch:hg-formulation} contains the details of \gls{hg} lifting and the necessary \emph{symbolic states}.
...
\end{description}
The vertices of \pgls{hg} are represented by the aforementioned symbolic states, which are tuples of type
\begin{equation*}
...
It is required because the algorithm explores the \gls{hg} state space by recursively adding new vertices/states and edges on the fly.
...
is computed until \pgls{lfp} is reached.
...
by defining $\gls{symbolic-state}[_0] \gls{order} \gls{symbolic-state}[_1]$ as $\gls{symbolic-state}[_1] = \gls{symbolic-state}[_0]\gls{join}\gls{symbolic-state}[_1]$.
Intuitively, $\gls{symbolic-state}[_0] \gls{order} \gls{symbolic-state}[_1]$ denotes that $\gls{symbolic-state}[_0]$ is ``less abstract'' than $\gls{symbolic-state}[_1]$.
...
\begin{equation}
  (\gls{concrete-state}\gls{holds}P \lor Q) \implies (\gls{concrete-state}\gls{holds}P\gls{join}Q).
\end{equation}
...
As \gls{order} is used as a termination condition during \gls{lfp} computation,
...
would result in \pgls{hg} that is too abstract and over which no properties could be proven.
...
\end{equation}
%\nomenclature[operator]{$\coloneqq$}{Indicates that the \gls{rhs} defines the \gls{lhs} for \gls{hg} and \gls{eicfg} work}%
...
%\nomenclature[operator]{$\in$}{Indicates that the \gls{lhs} is an element of the set on the \gls{rhs}}
where $\square\in\{=,\neq,<,<_s,\ge,\ge_s\}$.
...
There are also two special clauseless predicates,~\gls{top} and~\gls{bot}.
Those special predicates respectively indicate always true (holds for any state) and always false (holds for no state).
\gls{bot} is also used to indicate an unknown~\gls{constant}. % For some reason \Gls{bot} isn't working
...
\begin{example}[Aliasing and non-aliasing]
...
\begin{definition}[Memory relations]\label{def:memory-relations}
...
The \gls{smt} solver/theorem prover \gls{z3} \autocite{de2008z3} is used to establish whether these ``necessarily''-relations hold for symbolic addresses\index{symbolic!address}
...
\begin{definition}[Memory tree insertion]\label{def:insert}
  ...
  \begin{align*}
    \gls{insertM}_\text{AL}(t_0,t_1,M) &\gls{defeq} \{(R_0\cup R_1, M') \cons M \mid M' \in \mathsf{fold}(\gls{insertM}, M_0 \cup M_1) \} \\
    ...
    \gls{insertM}_\text{CON}(t_0,t_1,M) &\gls{defeq} \{\gls{insertM}[(t',M)]\mid t'\in\{\langle R_0,M'\rangle\mid M'\in \gls{insertM}[(t_1,M_0)]\}\}
  \end{align*}
\end{definition}
...
\end{enumerate}
\begin{example}[Potential aliasing in assembly]\label{ex:example_snippet}
...
\begin{definition}[Memory model holding]
  ...
  \end{equation*}
\end{definition}
\begin{example}[Memory model holding]
...
\begin{lemma}[Region insertion and memory model relations]\label{lem:insert}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    The \gls{ih} then finishes the proof.
    ...
    Region $r_0$ is recursively inserted into a child of $t_1$, proof follows from the \gls{ih}.
    ...
    Again, the \gls{ih} then finishes the proof.
...
\begin{definition}[Memory model join]\label{def:mem-join}
  ...
  \begin{align*}
    M_0\gls{join}M_1 &\gls{defeq} \left\{\gls{join}(T) \relmiddle| T\in\faktor{M_0 \cup M_1}{\gls{comparable-plus}}\right\}\notag \\
...
equivalence relation~\gls{comparable-plus}.
...
\begin{example}[Memory model join]
...
\begin{figure}
  \hspace*\fill
  \subcaptionbox{Alternate memory model\label{fig:otherMem}}[\widthof{(a) Alternate memory model}]{
    \begin{tikzpicture}[every node/.style={draw}]
      \node (a) {$\{\region\rdio8,\region\rdxo8\}$};
    \end{tikzpicture}
  }\hfill
  \subcaptionbox{Join result\label{fig:otherMemJoin}}[\widthof{(b) Join result}]{
    \begin{tikzpicture}[every node/.style={draw}]
      \node (a) {$\region\rdio8$};
    \end{tikzpicture}
  }
...
\end{figure}
\begin{example}[Memory model join cases]
...
\begin{lemma}[Soundness of the memory model join]\label{lem:mem_soundness}
...
\begin{definition}[Symbolic state join]
...
\Cref{alg:invgen} provides the base functionality for \gls{hg} extraction.
...
Second, the current \gls{hg} \gls{hg-var}.
When the \gls{bag} is empty, \gls{hg-var} is the final \gls{hg}.
...
\begin{algorithm}
  \caption{Base version of \glsfmtshort{hg} extraction}
  \label{alg:invgen}
  \algnewcommand{\IIf}[2]{\State\algorithmicif\ #1\ \algorithmicthen~#2}
  \begin{algorithmic}[1]
    \Function{explore}{}
      \State pop \gls{symbolic-state} from \gls{bag}\label{line:pop}
      \If{$\exists\gls{symbolic-state}[_c]\in\gls{hg-var}\sepdot \gls{symbolic-state}[_c]\gls{compatible}\gls{symbolic-state}$}\label{line:compat}
        ...
        \State $\gls{hg-var}{}[\gls{symbolic-state}[_c]\coloneqq\gls{symbolic-state}[_j]]$
      \Else
      ...
      \ForAll{$\gls{symbolic-state}[']\in \gls{step-hg}(\gls{symbolic-state}[_j])$}\label{line:forall}
        \State $\gls{hg-var}\gls{pluseq}(\gls{symbolic-state}[_j], \gls{symbolic-state}['])$
...
Function \gls{explore} first searches for a current symbolic state~$\gls{symbolic-state}[_c]$ already in the current~\gls{hg-var} that is compatible (\cref{line:compat}).
...
The~\gls{hg-var} is modified by replacing the current state with the joined one.
...
If no compatible state exists in the current~\gls{hg-var}, then~\gls{symbolic-state} is the state to be explored further (\cref{line:nocompat}).
Exploration occurs at \crefrange{line:forall}{line:endforall}.
For every successor~$\gls{symbolic-state}[']$ (possibly none), an edge is added to the~\gls{hg-var}.
...
To formulate soundness and present a proof, we first define a relation \gls{related} between the concrete transition system and the \gls{hg}.
...
\begin{definition}[Related to]
  A concrete state~\gls{concrete-state} is \emph{related to} symbolic state $\gls{symbolic-state}=\langle P,M \rangle$, notation $\gls{concrete-state}\gls{related}\gls{symbolic-state}$, if and only if:
  \begin{equation*}
    \gls{concrete-state}\gls{related}\gls{symbolic-state}\gls{defeq} (\gls{concrete-state}\gls{holds}P) \land (\gls{concrete-state}\gls{holds}M)
  \end{equation*}
\end{definition}

\begin{lemma}[''Related to'' is a simulation]\label{lem:simulation}
  ...
  \end{equation*}
  Then relation \gls{related} is a simulation between the concrete transition system and the transition system obtained by abstract step function \gls{step-hg}:
  \begin{equation*}
    \forall\gls{concrete-state}~\gls{concrete-state}['] \sepdot \gls{concrete-state} \gls{black-box-transition} \gls{concrete-state}['] \land \gls{concrete-state}\gls{related}\gls{symbolic-state}\implies \exists \gls{symbolic-state}[']\in \gls{step-hg}[(\gls{symbolic-state})] \sepdot \gls{concrete-state}[']\gls{related}\gls{symbolic-state}[']
  \end{equation*}
\end{lemma}
\begin{proof}
...
\begin{definition}[\Glsfmtshort{hg} soundness]
  The \gls{hg} $H = \langle\gls{abs-state},\gls{symbolic-state}[_I], \gls{symbolic-transition}\rangle$ is \emph{sound} with respect to some binary $B = \langle a_e, \gls{fetch}, \gls{conc-state},\gls{black-box-transition}\rangle$, notation $\gls{sound}(H,B)$, if and only if:
  \begin{equation*}
    \gls{sound}(H,B) \gls{equiv} \forall s_0 \gls{black-box-transition-star} \gls{concrete-state} \gls{black-box-transition} \gls{concrete-state}['] \sepdot \exists \gls{symbolic-state} \gls{symbolic-transition}[{}] \gls{symbolic-state}['] \sepdot \gls{concrete-state}\gls{related}\gls{symbolic-state} \land \gls{concrete-state}[']\gls{related}\gls{symbolic-state}[']
  \end{equation*}
\end{definition}
In words, for every reachable transition from~\gls{concrete-state} to~$\gls{concrete-state}[']$ in the binary, there must exist a related transition in the \gls{hg}.

\begin{theorem}[Algorithm soundness]\label{thm:algo_soundness}
  \Cref{alg:invgen} constructs a sound \gls{hg}.
\end{theorem}
\begin{proof}
  The structure of the algorithm is close to \pgls{dfs}.
  For that reason, the white-path lemma is used to prove soundness \autocite{cormen2009introduction}.
  For a normal \gls{dfs}, the white-path lemma states that the \gls{dfs} will eventually explore some state~$\gls{concrete-state}[']$ if and only if there exists some state~\gls{concrete-state} currently in the bag \emph{and} there exists a ``white'' path from~\gls{concrete-state} to~$\gls{concrete-state}[']$.
  A key difference between \cref{alg:invgen} and a normal \gls{dfs} is that states are joined.
  For the sake of this proof, a state is therefore considered ``white'' if the current \gls{hg} contains no compatible state that is equal or more abstract (under~\gls{order}).
  ...
  Let~\gls{symbolic-state} be the symbolic state related to concrete state~\gls{concrete-state}, i.e., $\gls{concrete-state}\gls{related}\gls{symbolic-state}$.
  Since~\gls{symbolic-state} is reachable, $\suprem(\gls{symbolic-state})$ is explored.
  We thus have
  $\gls{concrete-state}\gls{related}\gls{symbolic-state} \implies \gls{concrete-state}\gls{related}\suprem(\gls{symbolic-state})$.
  ...
  By \cref{lem:simulation}, we have $\gls{concrete-state}[']\gls{related}\gls{symbolic-state}[']$.
\end{proof}
...
%This is specific to the \gls{elf} format.
...
We make the assumption that this unknown function adheres to the 64-bit System~V \gls{abi}['s] calling convention.
  ...
  \begin{tabular}{c}\hline
    \begin{minipage}{0.94\linewidth}
      \begin{algorithmic}[1]
        ...
        \State \algorithmicif\ {$\exists \gls{symbolic-state}[_c] \in\gls{hg-var}\sepdot \gls{symbolic-state}[_c]\gls{compatible}\gls{symbolic-state} \land \mathsf{reachable}(\gls{symbolic-state}[_c])$}\ \algorithmicthen
      \end{algorithmic}
    \end{minipage}
    \\\hline
  \end{tabular}
