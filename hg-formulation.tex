...
Function \textproc{explore} is repeatedly called until that \gls{bag} is empty.
Second, the current \ac{hg} \gls{hg}.
When the \gls{bag} is empty, \gls{hg} is the final \ac{hg}.

Furthermore, the algorithm requires a function~\gls{step-hg} that models instruction semantics.
...
function~\gls{step-hg} transforms its supplied predicate
...
The memory model allows~\gls{step-hg} to take into account information on pointer relations
...
To this end, it requires the predicate of the current symbolic state.
...
\begin{definition}
  ...
  \begin{equation*}
    \begin{array}{l}
      ~~\step_\Sigma(\sigma)\gls{defeq}\{ \langle P',M'\rangle \mid P' \in \glsxtrfmt{step-hg}{P,M'} \wedge M' \in \insertM(R, M) \} \\
...
If the current predicate does not contain sufficient information to evaluate the region, evaluation produces~\gls{bot} and the region is not inserted.
...
Function \textproc{explore} first searches for a current symbolic state $\sigma_c$ already in the current \gls{hg} that is compatible (\cref{line:compat}).
...
an edge is added to the \gls{hg}.
...
\begin{lemma}\label{lem:simulation}
  Assume that predicate transformation~\gls{step-hg} is correct:
  \[
  \forall s~s' \sepdot s \rightarrow_B s' \wedge (s\gls{holds}P) \implies \exists Q\in\glsxtrfmt{step-hg}{P,M} \sepdot s'\gls{holds}Q
  \]
...
\begin{proof}
  ...
  By correctness of~\gls{step-hg}, we obtain a predicate $Q\in\glsxtrfmt{step-hg}{P,M}$ such that $s'\gls{holds}Q$.
  By \cref{lem:insert} (completeness of the insertion function), the memory model that holds in state $s'$ is generated.
  Since the step function overapproximates by taking any combination of predicates in $\glsxtrfmt{step-hg}{P,M}$ and generated memory models, there exists at least one symbolic state that is related to~$s'$.
\end{proof}
...
This is unsatisfactory for two reasons: first, for \emph{external} function calls, a function~\gls{step-hg} that transforms the predicate may not be available.
...
Function $\step_\Sigma$ therefore modifies the current state by assigning~\gls{bot} to all registers, flags and heap regions currently in the state that may not be assumed to be preserved by a function call.
