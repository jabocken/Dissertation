...
In this \namecref{ch:hg-formulation}, you will learn about the details of \ac{hg} lifting and the \emph{symbolic states} contained within.
...
In particular, he was a significant contributor to the proofs in this \namecref{ch:hg-formulation}.

\section{Starting Concepts}\label{hg-concepts}
\todo{maybe move some of this to an earlier/communal chapter?}
For this \namecref{hg}, we have the following types:
\todo{how to align these again?}
...
Moving on, the algorithm requires the definition of a \emph{join} operation over symbolic states.
...
\begin{equation}
  (\gls{concrete-state}\gls{holds}P \vee Q) \implies (\gls{concrete-state}\gls{holds}P\sqcup Q).
\end{equation}
...
\nomenclature{\gls{concrete-state}}{A concrete state \todo{give better clarification for this elative to the Hoare Graphs; gets introduced earlier though so maybe not?}}
...
\begin{lemma}\label{lem:pred_soundness}
  $\gls{concrete-state}\gls{holds}P\vee Q\implies s\gls{holds}P\gls{join}Q$
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:insert}
  ...
  \begin{equation*}
    \exists\gls{concrete-state} \sepdot \gls{concrete-state} \gls{holds}M \wedge (\forall r' \in R(M) \sepdot \gls{concrete-state} \models r_0~f(r')~r')
  \end{equation*}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item As an extra case, consider when no relation can necessarily be established. In that case, all of the above proofs apply, as memory models are generated for each possible case. \todo{Shouldn't I include this part? Not sure why it was commented out.}\qedhere
  \end{enumerate}
\end{proof}
...
\begin{definition}
  ...
  \begin{equation*}
    \begin{array}{l}
      ~~\gls{step-hg}[(\gls{symbolic-state})]\gls{defeq}\{ \langle P',M'\rangle \mid P' \in \gls{step-hg-semantics}[(P,M')] \wedge M' \in \gls{insertM}[(R,M)] \} \\
...
\begin{definition}
  ...
  \begin{equation*}
    \gls{concrete-state}~\mathbf{R}~\gls{symbolic-state}\gls{defeq} (\gls{concrete-state}\gls{holds}P) \wedge (\gls{concrete-state}\gls{holds}M)
  \end{equation*}
\end{definition}
...
\begin{lemma}\label{lem:simulation}
  ...
  \begin{equation*}
    \forall\gls{concrete-state}~\gls{concrete-state}['] \sepdot \gls{concrete-state} \gls{black-box-transition} \gls{concrete-state}['] \wedge (s\gls{holds}P) \implies \exists Q\in\gls{step-hg-semantics}[(P,M)] \sepdot s'\gls{holds}Q.
  \end{equation*}
  ...
  \begin{equation*}
    \forall\gls{concrete-state}~\gls{concrete-state}['] \sepdot \gls{concrete-state} \gls{black-box-transition} \gls{concrete-state}['] \wedge \gls{concrete-state}~\mathbf{R}~\gls{symbolic-state}\implies \exists \gls{symbolic-state}[']\in \gls{step-hg}[(\gls{symbolic-state})] \sepdot \gls{concrete-state}[']~\mathbf{R}~\gls{symbolic-state}[']
  \end{equation*}
\end{lemma}
\begin{proof}
...
\begin{definition}
  ...
  \begin{equation*}
    \gls{sound}(H,B) \gls{equiv} \forall s_0 \gls{black-box-transition}[^\ast] \gls{concrete-state} \gls{black-box-transition} \gls{concrete-state}['] \sepdot \exists \gls{symbolic-state} \gls{symbolic-transition} \gls{symbolic-state}['] \sepdot \gls{concrete-state}~\mathbf{R}~\gls{symbolic-state} \wedge \gls{concrete-state}[']~\mathbf{R}~\gls{symbolic-state}[']
  \end{equation*}
\end{definition}
...
\begin{proof}
  ...
  \begin{equation*}
    \begin{array}{l}
      ...
      \textbf{where}
      \\
      \suprem(\gls{symbolic-state}[']) \gls{equiv} \bigsqcup \{ \gls{symbolic-state}[''] \mid \gls{symbolic-state}['']\gls{compatible}\gls{symbolic-state}['] \wedge \exists \pi = [\gls{symbolic-state}, \dotsc, \gls{symbolic-state}['']] \sepdot \mathsf{white}(\pi) \}
    \end{array}
  \end{equation*}
  ...
  \begin{tabular}{c}\hline
    \begin{minipage}{0.94\linewidth}
      \begin{algorithmic}[1]
        ...
        \State \algorithmicif\ {$\exists \gls{symbolic-state}[_c] \in\gls{hg}\sepdot \gls{symbolic-state}[_c]\gls{compatible}\gls{symbolic-state} \wedge \mathsf{reachable}(\gls{symbolic-state}[_c])$}\ \algorithmicthen
      \end{algorithmic}
    \end{minipage}
    \\\hline
  \end{tabular}
