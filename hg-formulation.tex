...
\section{Algorithm}
\todo{I am here now; should this be its own chapter?}

\Cref{alg:invgen} provides the base functionality of \ac{hg} extraction.
This base functionality is extended in two ways:
\begin{enumerate}
  \item a context-free approach to function calls (see \cref{subsec:function_calls}) and
  \item preventing states from being joined when they are incompatible (see below).
\end{enumerate}
The algorithm maintains two objects.
First, a \emph{bag} of symbolic states that is to be explored.
Function \textproc{explore} is repeatedly called until the bag is empty.
Second, the current Hoare Graph $\var{HG}$.

The algorithm requires a function~$\tau$ modeling instruction semantics.
Our implementation supports a wide range of x86-64 instructions,
...
function~$\tau$ transforms its supplied predicate
into a set of predicates
by symbolically executing the single instruction.
The memory model allows~$\tau$ to take into account information on pointer relations
when performing symbolic execution using destination operands
that reference memory locations.


The algorithm requires an expression evaluation function
$\gls{eval}:\gls{expression}\times\gls{pointer}\mapsto\gls{constant}$,
which maps an expression (over registers, flags, and dereferenced memory regions)
to a constant expression.
To this end, it uses the predicate of the current symbolic state.
No memory model information is required
for this evaluation; that information is only required
when writing to a location in memory.
...
\begin{definition}
  The \emph{symbolic state step function} for symbolic state $\sigma = \langle P,M\rangle$, notation $\step_\Sigma(\sigma)$, is defined as:
  \begin{equation*}
    \begin{array}{l}
      ~~\step_\Sigma(\sigma)\gls{defeq}\{ \langle P',M'\rangle \mid P' \in \tau(P,M') \wedge M' \in \insertM(R, M) \} \\
      \mbox{\normalfont{\textbf{where}}}\\
      ~~\begin{aligned}
        R &\gls{defeq} \{ [\gls{eval}(a, P),s] \mid [a,s]\text { used by instruction } i \} - \{\bot\}\\
        i &\gls{defeq} \gls{fetch}(\gls{eval}(\rip, P))
      \end{aligned}
    \end{array}
  \end{equation*}
\end{definition}
Given the current symbolic state $\sigma$, the set of next symbolic states is obtained by applying predicate transformation to the current predicate and by inserting regions into the current memory model.
The set of regions is obtained by considering the operands of the current instruction.
For example, the instruction \inlineasm{mov qword ptr [rax + 4*rdi], rax} results in one region $\region{\rax + 4*\rdi}8$.
That region is -- given the current predicate -- evaluated to a constant.
For example, if the current predicate contains $\rax = 0x100$ and $\rdi = \rsio$, then evaluation produces the constant $0x100 + 4*\rsio$.
The evaluated region is inserted.
If the current predicate does not contain sufficient information to evaluate the region, evaluation produces~$\bot$ and the region is not inserted.
The latter overapproximates any relation (e.g., aliasing, separation) the new region may have with the current memory model.
Finally, if no bounded set of next states can be determined
(such as for unresolved indirect jumps or calls),
...
A second extension to the base algorithm modifies this definition so that states are not considered compatible when registers contain different immediate values that directly influence control flow (e.g., when the immediates are loaded from a jump table).
In general, it is impossible to know whether a stored value will influence future control flow.
However, it suffices to detect situations in which values will \emph{likely} influence future control flow.
If a value was erroneously deemed to influence future control flow, then we have unnecessarily explored paths that could have been joined earlier, but this only affects run-times and not soundness.
If we join states that contain immediate values that turn out to be necessary to assess future control flow, this will lead to unresolved indirections or a verification error, but does not affect soundness.
...
Let $\sigma$ be some symbolic state from the bag (\cref{line:pop}).
Function \textproc{explore} first searches for a current symbolic state $\sigma_c$ already in the current $\var{HG}$ that is compatible (\cref{line:compat}).
If such a state exists and it is more abstract (based on ordering $\sqsubseteq$) than state $\sigma$, no further exploration is necessary (\cref{line:terminate}).
Otherwise, $\sigma$ and~$\sigma_c$ are joined (Line~\ref{line:join}).
The $\var{HG}$ is modified by replacing the current state with the joined one.
...
If no compatible state exists in the current $\var{HG}$, then $\sigma$ is the state to be explored further (\cref{line:nocompat}).
Exploration occurs at Lines~\ref{line:forall} to~\ref{line:endforall}.\todo{cref should have a way to do this}
For every successor $\sigma'$ (possibly none),
an edge is added to the $\var{HG}$.
If, for some successor evaluation of the instruction pointer, $\rip$ does not produce an immediate concrete value, then this is due to either 1.) a return statement (after which \reg{rip} is set to the symbol pushed to the top of the stack in the initial state), or 2.) the current symbolic state does not provide sufficient information to resolve the computation of \reg{rip} (because of an indirect branch, for example). In the second case, the state is annotated with an unsoundness warning (\cref{line:undsound})
and the algorithm terminates early.
Otherwise, the successor is added to the bag.

\paragraph{Soundness}
\todo{Another relation to glossarify}
To formulate soundness and present a proof, we first define a relation $\mathbf{R}$ between the concrete transition system and the Hoare Graph.
We then prove Lemma~\ref{lem:simulation}, which shows that this relation is a simulation.
As a direct result of this lemma, any concrete path can be simulated by a path consisting of symbolic steps produced by function $\step_\Sigma$.
...
\begin{lemma}\label{lem:simulation}
  Assume that predicate transformation $\tau$ is correct:
  \[
  \forall s~s' \sepdot s \rightarrow_B s' \wedge (s\gls{holds}P) \implies \exists Q \in \tau(P,M) \sepdot s'\gls{holds}Q
  \]
  Then relation $\mathbf{R}$ is a simulation between the concrete transition system and the transition system obtained by abstract step function $\step_\Sigma$:
  \[
  \forall s~s' \sepdot s \rightarrow_B s' \wedge s~\mathbf{R}~\sigma \implies \exists \sigma' \in \step_\Sigma(\sigma) \sepdot s'~\mathbf{R}~\sigma'
  \]
\end{lemma}
\begin{proof}
  Let $s$ and $\sigma$ be two related states.
  Hence $(s\gls{holds}P) \wedge (s\gls{holds}M)$.
  By correctness of $\tau$, we obtain a predicate $Q \in \tau(P,M)$ such that $s'\gls{holds}Q$.
  By \cref{lem:insert} (completeness of the insertion function), the memory model that holds in state $s'$ is generated.
  Since the step function overapproximates by taking any combination of predicates in $\tau(P,M)$ and generated memory models, there exists at least one symbolic state that is related to~$s'$.
\end{proof}
...
\begin{theorem}\label{thm:algo_soundness}
  Algorithm~\ref{alg:invgen} constructs a sound Hoare Graph.
\end{theorem}
\begin{proof}
  The structure of the algorithm is close to a depth-first search (DFS).
  For that reason, the white-path lemma is used to prove soundness~\cite{cormen2009introduction}.
  For a normal DFS, the white-path lemma states that the DFS will eventually explore some state $s'$ if and only if there exists some state $s$ currently in the bag \emph{and} there exists a ``white'' path from $s$ to $s'$.
  A key difference between Algorithm~\ref{alg:invgen} and a normal DFS is that states are joined.
  For the sake of this proof, a state is therefore considered ``white'' if the current HG contains no compatible state that is equal or more abstract (under $\sqsubseteq$).
  We reformulate the white-path lemma as follows:
  \[
  \begin{array}{l}
    \indent\mathsf{sup}(\sigma') \text{ is explored } \Longleftrightarrow \\
    \indent\exists \sigma \in \bag \sepdot \exists \pi = [\sigma, \dotsc, \sigma'] \sepdot \mathsf{white}(\pi)
    \\
    \textbf{where}
    \\
  ...
  Lemma~\ref{lem:simulation} shows that the path from $s_0$ to $s$ can be simulated by a path of related symbolic states.
  ...
  This is a direct implication of Lemma~\ref{lem:mem_soundness}: since joining makes the states more abstract, it makes the set of related concrete states larger.
  Line~\ref{line:forall} will then explore some state $\sigma' \in \step_\Sigma(\sigma_j)$.
  By Lemma~\ref{lem:simulation}, we have $s'~\mathbf{R}~\sigma'$.
\end{proof}

\subsection{Extension: Function Calls}\label{subsec:function_calls}
The base algorithm as presented in Algorithm~\ref{alg:invgen} does not treat function calls as special instructions.
This is unsatisfactory for two reasons: first, for \emph{external} function calls, a function $\tau$ that transforms the predicate may not be available.
...
The function name is matched against a list of hard-coded function names that are known to be terminating, such as \texttt{exit} and \texttt{stack\_chk\_fail}.
In case of a terminating function, function $\step_\Sigma$ will produce the empty set, stopping further exploration from the current state.
Otherwise, the function is some unknown external function. % whose behavior cannot be known.
We make the assumption that this unknown function adheres to the 64-bit System V calling convention.
Function $\step_\Sigma$ therefore modifies the current state by assigning $\bot$ to all registers, flags and heap regions currently in the state that may not be assumed to be preserved by a function call.
...
%This only removes information.
As with the join operation, this usage is sound as the end result is always a weakening of the postcondition.

\subsubsection{Internal Functions}
If the operand of a \texttt{call} can be resolved to an address inside the executable range of the binary, it is recognized as an internal call.
...
Each symbolic state has a Boolean field that is set to true only if the state is known to be reachable.
  ...
  \begin{tabular}{c}\hline
    \begin{minipage}{0.94\linewidth}
      \begin{algorithmic}[1]
        ...
        \State \algorithmicif\ {$\exists \sigma_c \in \var{HG} \sepdot \sigma_c\gls{compatible}\sigma \wedge \mathsf{reachable}(\sigma_c)$}\ \algorithmicthen
      \end{algorithmic}
    \end{minipage}
    \\\hline
  \end{tabular}
...
\noindent
Moreover, after Line~\ref{line:addbag}, any symbolic state with the same \texttt{rip} as the newly explored $\sigma'$ is marked as reachable:
\\
  ...
  \begin{tabular}{c}\hline
    \begin{minipage}{0.94\linewidth}
      \begin{algorithmic}[1]
        ...
        \State mark all $\sigma \in \bag$ with $\texttt{rip}(\sigma) = \texttt{rip}(\sigma')$ as reachable
      \end{algorithmic}
    \end{minipage}
    \\\hline
  \end{tabular}
...
As soon as the instruction pointer is set to symbol $\mathcal{S}_\mathtt{0x400}$ (e.g., by a \texttt{ret} instruction), all mapped return addresses are set to reachable.
