...
Characterizing the regions in memory that are read and written by a program
allows reasoning over those regions.
When you can reason over those regions, you can prove \emph{memory preservation}:
that the values written by that program are constrained to its memory regions
and it does not modify any other regions in memory.
When you can prove memory preservation, you can work towards even larger analyses.
Of course, that first requires being able to prove that property.
...
but it is not a feasible approach.
As per Rice's theorem \autocite{rice1953classes}, memory preservation is ultimately an undecidable property.
One alternative would be to use \ac{itp}, but that does not scale for large systems either.
...
is the best way to go. This does require a significant amount of up-front verification work, however.
...
Properties that reason over the concrete memory used by a program, such as memory preservation,	
...
allows bypassing the build process, which also opens the door for verification of legacy code.
...
that has been formally proven to maintain the semantics of the source code in the binaries it produces.
...
assembly code generally lacks the structured control flow found in languages on a higher level of extraction.
...
Some years ago, \textcite{shi2012orientais} provided a verification methodology
...
A little later, \textcite{goel2014syscalls,goelphd} produced formal semantics
...
That is only \SI{28}{\percent} of the functions, or \SI{26.5}{\percent} of the instructions,
that we verified here.
On top of that, the \acp{isa} they worked with are not as low-level as the \arch\ \ac{isa}.
While they verified functional correctness instead of a weaker property like memory preservation,
they also specifically reduced the complexity of the most complicated set of functions they verified
