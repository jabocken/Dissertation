...
\section{Static Assembly and Binary Analysis}\label{static-analysis}
...
The process of \emph{decompilation} is the reverse of compilation \todo{citation?}.
It attempts to \emph{lift} low-level code, possibly even machine code, to a higher-level representation.
That representation can be something like LLVM\index{LLVM} \ac{ir} or bitcode \autocite{dinaburg2014mcsema}, \gls{c} \autocite{brumley2013native}, or even \Cpp\ \autocite{fokin2011smartdec}.
...
Generally they lift to \iac{ir} that is then verified \autocite{myreen2007hoare,myreen2012dil,brumley2011bap}.
...
If you are instrumenting binaries to dynamically randomize their own basic blocks, you cannot do so without knowing the bounds those blocks must abide by \autocite{wartell2012binary}.
\autocite{,kim2017revarm}
...
It is also restricted specifically to insertion of code trampolines \todo{citation or glossary entry}.
...
\subsubsection{Security}
General security analysis also benefits from control flow information.

\autocite{kruegel2005automating,song2008bitblaze,davi2009dynamic,wang2017angr}
