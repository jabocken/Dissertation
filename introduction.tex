...
In order to have a sound and thus \emph{overapproximative}
-+
While there are many tools that perform this sort of control flow recovery, they all have some limitations.
They may operate in an unsound fashion are not overapproximative.
Alternatively, they may only cover certain types of control flow in a binary.

\section{Motivation}
\todo{further paragraph on memory preservation?}
...
\todo{flesh out the uses mentioned above and move the discussion of difficulty down into challenges}
...
\emph{no existing works could provide scalable, formally overapproximative
...
% todo: this can probably just go in the HG part
Moving on, \cref{hg-example} contains an example where instructions are potentially overlapping, which is often an arrangement found in obfuscated code.
...
None of the contributions in this dissertation cover the usage of concurrency.
We have scoped it out due to its complicated nature.
\todo{more}

\todo{targeting \gls{arch} assembly in all cases; targeting the System~V \ac{abi} and \ac{elf} binaries for the second half of the contributions}

Due to the aforementioned undecidability, none of these approaches are universal.
