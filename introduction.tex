...
It also introduces the concepts of \emph{\acp{hg}} and \emph{\acp{eicfg}}, which deal with complex, interprocedural control flow.
...
Full automation can, however, be supported with a narrowing of scope, which is where the concepts of \acp{hg}, which operate on programs with indirect control flow, and \acp{eicfg}, which additionally operate on programs with \Cpp-style exception handling \autocite{cxxEhAbi}, come into play.
Those constructs are targeted specifically at control flow reconstruction of binaries.

Formal verification of software has been an active research field for decades.
This dissertation aims to provide a formal verification method for real-world programs that is specifically tailored to memory preservation.
...
obtained by disassembling binaries of the Xen hypervisor \autocite{xen} with minimal user interaction.

It also aims to provide methods for lifting control flow from binaries even in the presence of indirect branches and other complex control flow constructs, such as exception handling.
These methods are fully automated and scalable, with the \ac{hg} work lifting \glssymbol{inst-total-lifted} instructions and the \ac{eicfg} work \glssymbol{eicfg-inst-total}.

To the best of our knowledge, there exist no current state-of-the-art methods that specifically aim to formally verify memory preservation, nor to lift binaries to \ac{eicfg}-like constructs.
\Ac{hg} lifting does have a similar work \autocite{\autocite{kinder2010static,kinder2012alternating,kinder2012virtualization}}, but it takes some different approaches and we found it to be rather limited practically.
...
\section{Motivation for Control Flow Lifting}
...
\subsection{Challenges for Memory Preservation}
...
This dissertation therefore proposes a \emph{semi-automated} approach.
It uses \iac{itp} environment,
but with code generation to generate as much of the proof code as possible.


\subsection{Challenges for Control Flow Lifting}\label{challenges-cfg}
...
Additionally, a few years ago, \textcite{fromherz2019verified} embedded a subset
...
\subsection{Current Approaches to Control Flow Lifting}
...
The soundness of this overapproximation is shown with pen-and-paper proofs in \cref{hg-formulation}.
