...
%Following that is work on a lower level, specifically formal analysis of \acp{isa} and lower-level hardware details, in \cref{se:hardware}.
...
Their work used \ac{spade}, % \autocite{carre1986spade}, % leaving out until I get rid of abbrevs; might even be able to leave out afterwards if I get glossaries working right
...
\begin{comment}
  ...
  This can involve validating properties of the \ac{isa}, or even lower-level properties.
  Quite often this is done with model checkers.

  For example, Arm\index{Arm Ltd.} recently released several of their \gls{arm} \acp{isa} in a machine-parsable, executable format called \ac{asl}, a result that took five years to develop.
  While not directly verifiable, the \ac{asl} documents were automatically translated into a verifiable form.
  ...
  ISA-Formal\index{ISA-Formal} is intended to verify processor pipeline control and has been successfully used for that purpose on multiple versions of the \gls{arm} \ac{isa}.
  At its core, it uses bounded model checking for instruction sequence exploration.

  Not all of the components they worked with could be handled by the model checkers, however; that usage was restricted to \ac{isa} analysis.
  Alternative verification techniques were used for the components that the model checkers could not handle, such as the \acp{fpu} and memory system.
  ...
  The \acp{isa} in question are \gls{arm}[v8.3-A], CHERI-\ac{mips}, and \gls{riscv}.
...
\begin{table*}
  ...
  \begin{tabular}{l l l l l}
    ...
    \midrule
%    Reid et al. & \acs*{asl} & \acs*{mc}+others & \gls{arm} \acp{isa} & \num{2209191}+ \acs{sloc} \\
...
Despite all this, a key factor in many decompilation approaches, and also in other approaches that aim at producing \acp{cfg} or dataflow analyses, is that they rely on the disassembly and \ac{cfr} being done by an external tool that is assumed to be sound.
...
Many loop-identifying algorithms used by decompilers require \iac{cfg} as well \autocite{hecht1974reducible,wei2007loops}.
...
\begin{comment}
  ...
  It uses abstract interpretation to refine \acp{cfg} from binaries for debloating and deals with indirect branches in an overapproximative fashion.
  ...
  This tool is part of the SeaHorn verification framework for \gls{llvm} \ac{ir} \autocite{seahorn}.
