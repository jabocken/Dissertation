...
\todo{mention stuff from HG, EICFG papers here}
...
\begin{remark}[Function Isolation]
...
\section{Lifting \Acs*{cfg} from Binaries}
\todo\dots
...
%Linear sweep disassemblers, such as GNU's \lstinline|objdump|, scan through a binary linearly
%in areas where code is typically encountered
%to extract instructions~\cite{schwartz2002disassembly}.
%Linear sweep may be unsound.
%For example, unsoundness may occur when working with variable-length instructions (such as x86), with ISAs that do not have strict alignment requirements and when a binary contains mixed code and data.
%No reachability analysis is done to determine which bytes are actually reachable.
%Concretely, it considers the instruction after a \texttt{call} reachable by default, without doing reachability analysis.
%In contrast, our technique does not analyze that instruction until it is known that the call returns.
%By following control flow rather than making a best guess of instruction addresses, our technique can handle variable-length instructions and any alignment.
...
%For example, cases where a byte-sequence may be executable in different ways depending on the exact location in that byte-sequence where execution starts (such as jumping to the start versus jumping into the middle).
%They will also exclude code that is not executed.
...
Probabilistic approaches to disassembly~\cite{khadra2016speculative,miller2019probabilistic} include machine learning techniques, such as BYTEWEIGHT~\cite{bao2014byteweight} or the works of Wartell et al.~\cite{wartell2011differentiating,wartell2014shingled}.
...
%The approach of Miller et al.~\cite{miller2019probabilistic} identifies the probability of byte sequences representing instructions based on the context.
%It uses manually derived \emph{hints} rather than training sets.
%Spedi provides a speculative, conflict analysis-based approach~\cite{khadra2016speculative}.
...
such as IDA Pro, % \fturl{https://www.hex-rays.com/products/decompiler/},
\todo{citation}
...
%For example, McSema requires an external source of control flow information, which can be generated from our solution.
...
It is a technique that can be used in formal verification contexts. % , as the functional representation is overapproximative.
...
\todo{seL4 citation or glosary entry?}
...
In contrast, our improved control flow extraction approach is targeted at the scenario where a higher-level artifact such as source code or a specification is not available.
In such contexts, most approaches are interactive \autocite{goel2014syscalls,goelphd,verbeek2019establishing}.
\todo{maybe flesh the Goel bits out again?}
\Textcite{verbeek2020automated} provided a binary verification methodology tailored to memory preservation properties, with a ``manual effort vs. instruction count ratio'' of roughly 1 to 11.\footnote{%
  This is the paper version of \cref{ch:syntax}.
}
\Textcite{tan2015auspice} provide a fully automated method, AUSPICE, that takes about \SI6\hour\ for a \num{533}-instruction string search algorithm.
Our approach \emph{complements} formal verifiers that generate invariants for proving functional correctness.
\begin{remark}[Lifting in the \acs*{tcb}]
  The invariants our approach generates do not contain enough information to prove full functional correctness; they are tailored for control flow and pointer relations.
...
Thus, approximate solutions have been extensively studied, especially in the framework of \emph{abstract interpretation} \autocite{cousot1977abstract}.
\todo{glossary entry?}
Abstract interpretation provides gives a mathematical foundation for reasoning over abstractions ov.

%Binary analysis is an actively studied application of abstract interpretation, which gives mathematical foundations to reason about approximations and their computations.
\Textcite{bardin2011bincoa,daniel2020binsec} provide Binsec, which utilizes abstract interpretation in the process of information flow analysis for cryptographic implementations.
\Textcite{zhang2019bda} provide a path sampling algorithm and use abstract interpretation to prune infeasible paths.
\Textcite{reinbacher2011test} use abstract interpretation in a similar fashion for binary-level test case generation.
These works aim at orthogonal usages of abstract interpretation with respect to this paper, and \emph{assume} availability of a tool that provides overapproximative control flow.
Our approach is thus highly complementary to all these works.

BinTrimmer, developed by \textcite{redini2019b}, uses abstract interpretation to refine \acp{cfg} from binaries for debloating.
Its deal with indirect branches in an overapproximative fashion.
The key distinction between \todo{the work presented in } and BinTrimmer is that it is \emph{solely} concerneed with indirections.
...
\todo{figure out if anything is shared with the above. Most don't seem to be, but still}
...
%This section focuses on bottom-up approaches.
...
% TODO: Even works that hijack runtime control flow to force specific path execution must do it in a sampled fashion: https://link.springer.com/chapter/10.1007/978-3-540-74320-0_12
\begin{table}
  \centering
...
\href{https://www.nongnu.org/libunwind/}{libunwind}. \todo{make a citation instead, clean up more}
...
\todo{Add in talk about HG, EICFG works}
