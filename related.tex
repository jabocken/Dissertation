...
%Following that is work on a lower level, specifically formal analysis of \glspl{isa} and lower-level hardware details, in \cref{se:hardware}.
...
%We subsequently discuss, to provide contrast, verified compilation and informal static analysis tools in \cref{se:verified,se:static_analysis}.
...
Switching tracks to a more structural organization for the second half of this dissertation, we have the following two \lcnamecrefs{related-lifting}.
...
analyzable subset of the \gls{8080} \gls{isa}.
Their work used \gls[prereset]{spade}, % resetting just in case
a set of software tools for ``the efficient development, analysis, and formal verification of high-integrity software''.
\Gls{spade} provides \pgls{fdl} for modeling programs
in order to analyze and formally verify them
using \pgls{vcg}, proof checker, and symbolic interpreter.
They used automatic translation to model their \gls{space8080} program
in \gls{fdl}, and their verification methodology used the same kind of
...
of the formal models in \gls{spade}. Unlike the work detailed in \cref{ch:cfg}, however,
they only covered a single 33-instruction function
with the verification methodology they presented.

Another usage of \gls{spade} for a more in-depth verification of assembly
...
It formally covers the gap between machine code and \pgls{hol} model
...
it utilizes \pgls{hoare} framework for its verification.
...
calculate the actual speed, and then transmit it over \pgls{can} bus.
...
\begin{remark}[Function isolation in our works]
  ...
  By contrast, the analyzed \glslink{xen}{Xen} functions in \cref{se:xen} were handled without any modification whatsoever to the \glslink{xen}{Xen} build process and covered even more instructions.
  Building on that, \cref{hg} had a combination of full-binary and per-function analyses with even more instructions for its \glslink{xen}{Xen} case study (\cref{hg-extraction}), while \cref{eicfg} was primarily per-binary but had unsound additional coverage via per-function fallbacks and went far beyond just \glslink{xen}{Xen} for its case study (\cref{eicfg-results}).
\end{remark}
...
then being supplied to \pgls{smt} solver.
...
\begin{comment}
  \section{ISA and Hardware-Level Verification}\label{se:hardware}
  On a level lower than the assembly code level, or even the machine code level, is the work done by hardware designers and testers to verify the products that run those codes.
  This can involve validating properties of the \gls{isa}, or even lower-level properties.
  Quite often this is done with model checkers.

  For example, Arm\index{Arm Ltd.} recently released several of their \gls{arm} \glspl{isa} in a machine-parsable, executable format called \gls{asl}, a result that took five years to develop.
  While not directly verifiable, the \gls{asl} documents were automatically translated into a verifiable form.
  Specifically, the subset of System-Verilog usable with commercial model checkers.
  This form was utilized by a tool called ISA-Formal\index{ISA-Formal} \autocite{reid2016endtoend}.
  ISA-Formal\index{ISA-Formal} is intended to verify processor pipeline control and has been successfully used for that purpose on multiple versions of the \gls{arm} \gls{isa}.
  At its core, it uses bounded model checking for instruction sequence exploration.

  Not all of the components they worked with could be handled by the model checkers, however; that usage was restricted to \gls{isa} analysis.
  Alternative verification techniques were used for the components that the model checkers could not handle, such as the \glspl{fpu} and memory system.
  These alternative techniques involved raising the abstraction level and/or reducing the explored state space,
  as their goal was not necessarily to detect all bugs in those specific units.
  Instead, they wanted to check the correctness of the logic connecting them to other processor elements, which they were successful at.

  More recently, the Sail \gls{isa} specification language,
  which supports automatic generation of emulation code
  and of proof definitions for \gls{isabelle}, \gls{hol4}, and in some cases \gls{coq},
  has been used to provide rigorous semantic models of various \gls{risc} \glspl{isa}
  \autocite{armstrong2018models,armstrong2019isa}.
  The \glspl{isa} in question are \gls{arm}[v8.3-A], CHERI-\gls{mips}, and \gls{riscv}.
...
\Textcite{shi2012orientais} formally verified \pgls{rtos} for automotive use
called ORIENTAIS.
...
Their methodology integrated \gls{hol4} with \gls[prereset]{bap}.
\Gls{bap} utilizes a custom intermediate language
that provides an architecture-agnostic representation of machine instructions
and their side effects.
First, the formal model of the \gls{arm} \gls{isa} provided by \textcite{fox2010arm} was used
in an \gls{hol4} tool to translate the \gls{arm} binary into \gls{bap}['s] intermediate language.
Following that, the \gls{smt} solver \gls{stp} was used to determine the targets of indirect branches
...
All of their work was implemented in \gls{nqthm}.
...
was an implementation of its formal \gls{isa}.
\Textcite{bevier1989short,bevier1989kit,bevier1987verified}
designed a small \gls{os} kernel, Kit, and proved that it implemented
...
\begin{comment}
  ...
  used by the \gls{sel4} project to reduce its \gls{tcb} \autocite{klein2014comprehensive}.
  ...
  Clight has been mechanized in the \gls{coq} theorem prover with established
  big-step operational semantics, making it a useful subset of C
  for program verification work in \gls{coq}.
  ...
  subset of Standard ML modeled with big-step operational semantics in \gls{hol}.
  Its main compiler frontend is designed to take \gls{ml}-like \gls{hol} functions
  and translate them to a CakeML \gls{ast}, which is then translated into machine code
  using a verified backend. The compiler itself is bootstrapped,
  meaning it can compile itself in \gls{hol}. It also provides support for using \gls{hoare-logic}
  ...
  \index{interrupt}
  \gls{os} kernels that use device drivers.%
  ...
  The verification was done in \gls{coq} with the kernel written in Clight.
  ...
  In order to deal with devices running in parallel with the \gls{cpu},
  ...
  is given its own logical \gls{cpu}, running independently from the core of the kernel.
  ...
  provides a tool called Vine that lifts \gls{x86} instructions to its own \gls{il} for assembly
  ...
  if so desired.
  In terms of analyses, Vine can construct \glspl{cfg} for the lifted programs,
  ...
  it does support interfacing with the aforementioned \gls{smt} solver \gls{stp}
  as well as CVC Lite \autocite{barrett2004cvcl} and CVC3 \autocite{barrett2007cvc3}.
  This allows for formal verification.

  % Infer \emph{is} formal, not sure why I put it here
  Meanwhile, the tool Infer \autocite{calcagno2011infer}, now developed at Facebook,
  provides in-depth static analysis of \gls{llvm} code to detect bugs in programs written in a variety of languages.
  ...
  in order to provide immediate feedback even in \gls{ci} scenarios.
  ...
  and checks for calls to mobile device \glspl{api} that are not available
  for the target device \gls{os}.
  In Android code and \gls{java} in general, it ensures annotations can be reached,
  looks out for mutability issues, and checks for resource leaks.

  The static analysis tool FindBugs, written for \gls{java} code,
  ...
  it searches \gls{java} bytecode for common (bad practice) code idioms
...
\begin{table*}
  ...
  \begin{tabular}{l l l l l}
    ...
    \midrule
%    Reid et al. & \glsxtrshort{asl} & \glsxtrshort{mc}+others & \gls{arm} \glsxtrshortpl{isa} & \num{2209191}+ \glsxtrshort{sloc} \\
...
%, and abstract interpretation (\cref{related-absint}), as well as ones that learn state machines from black-box implementations (\cref{related-automata}).
...
\begin{table*}
  ...
  \begin{tabular}{lllll}
    ...
    \textbf{\Cref{hg}} & Yes & Yes & Non-det.\ mem+join & \glslink{xen}{Xen Project} \\
...
%abstract interpretation, like those works in \cref{related-absint}.
...
Also, Jakstab takes a slightly different approach to calculating \glspl{lfp}.
...
Many loop-identifying algorithms used by decompilers require \pgls{cfg} as well \autocite{hecht1974reducible,wei2007loops}.
...
In contrast, our \gls{hg} lifting approach is targeted at the scenario where a higher-level artifact such as source code or a specification is not available.
...
\begin{remark}[Lifting in the \glsfmtshort{tcb}]
...
\begin{comment}
  ...
  These works aim at orthogonal usages of abstract interpretation with respect to our \gls{hg} lifting work, as they \emph{assume} availability of a tool that provides overapproximative control flow.
  Our approach is thus highly complementary to them.

  There is also BinTrimmer, developed by \textcite{redini2019b}.
  It uses abstract interpretation to refine \glspl{cfg} from binaries for debloating and deals with indirect branches in an overapproximative fashion.
  The key distinction between our \gls{hg} approach and BinTrimmer is that it is \emph{solely} concerned with indirections.
  ...
  Moreover, BinTrimmers' case studies are six hand-picked binaries containing up to \num{555} \gls{sloc}, in contrast to the \gls{hg} \glslink{xen}{Xen hypervisor} case study presented in \cref{hg-extraction}.
  ...
  This tool is part of the SeaHorn verification framework for \gls{llvm} \gls{ir} \autocite{seahorn}.
...
While Binary Ninja \autocite{binary-ninja}, IDA Pro \autocite{ida}, and Ghidra \autocite{ghidra} all support some form of intraprocedural \gls{eh} analysis, they can only perform interprocedural \gls{eh} analysis dynamically via debugging.
...
Our final tool, as well as all of the other bottom-up analysis tools mentioned above, assumes both the existence and correctness of the \inlineasm{.eh_frame} and \inlineasm{.gcc_except_table} \gls{elf} sections.
...
%except the work produced by Arm\index{Arm Ltd.} employees.
